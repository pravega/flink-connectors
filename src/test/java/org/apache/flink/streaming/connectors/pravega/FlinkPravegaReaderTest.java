...
import org.apache.flink.streaming.connectors.pravega.utils.StreamSourceOperatorTestHarness;
import org.apache.flink.streaming.connectors.pravega.watermark.AssignerWithTimeWindows;
import org.apache.flink.streaming.connectors.pravega.watermark.LowerBoundAssigner;
...
public class FlinkPravegaReaderTest {
    ...
    public void testRun() throws Exception {
        ...
        try (StreamSourceOperatorTestHarness<Integer, TestableFlinkPravegaReader<Integer>> testHarness =
            ...
            MetricGroup pravegaReaderMetricGroup = testHarness.getMetricGroup().addGroup(FlinkPravegaReader.PRAVEGA_READER_METRICS_GROUP);
            MetricGroup readerGroupMetricGroup = pravegaReaderMetricGroup.addGroup(FlinkPravegaReader.READER_GROUP_METRICS_GROUP);
            String scopeString = ScopeFormat.concat('.', readerGroupMetricGroup.getScopeComponents());

            validateMetricGroup(scopeString, FlinkPravegaReader.UNREAD_BYTES_METRICS_GAUGE, readerGroupMetricGroup);
            validateMetricGroup(scopeString, FlinkPravegaReader.READER_GROUP_NAME_METRICS_GAUGE, readerGroupMetricGroup);
            validateMetricGroup(scopeString, FlinkPravegaReader.UNREAD_BYTES_METRICS_GAUGE, readerGroupMetricGroup);
            validateMetricGroup(scopeString, FlinkPravegaReader.ONLINE_READERS_METRICS_GAUGE, readerGroupMetricGroup);
            validateMetricGroup(scopeString, FlinkPravegaReader.UNREAD_BYTES_METRICS_GAUGE, readerGroupMetricGroup);

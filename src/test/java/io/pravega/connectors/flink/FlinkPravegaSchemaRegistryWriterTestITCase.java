    ...
    private <T> EventStreamReader<T> getReader(String streamName, Class<T> tClass) {
        ...
        try (ReaderGroupManager readerGroupManager = ReaderGroupManager.withScope(PRAVEGA.operator().getScope(), PRAVEGA.operator().getClientConfig())) {
            readerGroupManager.createReaderGroup(
                    readerGroupName,
                    ReaderGroupConfig.builder().stream(Stream.of(PRAVEGA.operator().getScope(), streamName)).build());
        }

        return EventStreamClientFactory.withScope(PRAVEGA.operator().getScope(), PRAVEGA.operator().getClientConfig()).createReader(
                RandomStringUtils.randomAlphabetic(20),
                readerGroupName,
                new DeserializerFromSchemaRegistry<>(PRAVEGA.operator().getPravegaConfig().withSchemaRegistryURI(

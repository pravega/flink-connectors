...
public class PravegaEventWriterTest {
    ...
    public void testNonTransactionalWriterWriting() throws Exception {
        ...
        try (OneInputStreamOperatorTestHarness<Integer, CommittableMessage<PravegaTransactionState>>
                     testHarness = createTestHarness(writer)) {
            testHarness.open();
    ...
    private static PravegaEventSink<Integer> mockSink(PravegaWriterMode writerMode,
                                               SinkWriter<Integer> writer) throws IOException {
        final PravegaEventSink<Integer> sink = spy(new PravegaEventSink<>(false,
        ...
        Mockito.doReturn(writer).when(sink).createWriter(anyObject());

        return sink;
    }

    /**
     * A test harness suitable for BEST_EFFORT and ATLEAST_ONCE tests.
     *
    ...
    createTestHarness(PravegaEventWriter<Integer> writer) throws Exception {
        return new OneInputStreamOperatorTestHarness<>(
                new SinkWriterOperatorFactory<>(mockSink(PravegaWriterMode.ATLEAST_ONCE, writer)),
